...
new_data <- with(consumer_comm_metrics, tibble(
  ...
  sample_year = levels(consumer_comm_metrics$sample_year)[[1]]
))
...
crit.t <- qt(0.975, df = df.residual(m0))
new_data <- new_data %>% 
  mutate(upper=fit+(crit.t*se.fit), lower=fit-(crit.t*se.fit))

# PLot predictions
model_label <- c("s(PC1, 1.993)","'Deviance expl.' == '40%'")
p.kud.c <- new_data %>% 
  ggplot(aes(x=PC1,y=fit)) + 
  geom_ribbon(aes(ymin=lower,ymax=upper, x = PC1), alpha=.5, fill="grey") + 
  geom_line() +
  ...
  # scale_y_continuous(limits=c(0,40), breaks = seq(0,40,10)) +
  labs(title = "", x = "Long. gradient (PC1)", 
       y = expression('Consumer trophic diversity ('~KUD[95]~')'), 
       fill = "Stream system") +
  annotate(geom = "text", x = 0.5, y = c(45, 45*0.9),
         hjust = 0, vjust = 1,  label = model_label, parse = TRUE, size = 3)
p.kud.c
...
model_label <- c("s(PC1, 1.00)","'Deviance expl.' == '10.6%'")
p.CR.c <- new_data %>% 
  ggplot(aes(x=PC1,y=fit)) + 
  # geom_ribbon(aes(ymin=lower,ymax=upper, x = PC1), alpha=.5, fill="grey") + 
  # geom_line() +
  ...
  # scale_y_continuous(expand=c(0,0), limits=c(0,10), breaks = seq(0,10,2)) +
  labs(title = "", x = "Long. gradient (PC1)", 
       y = expression('Consumer '~{delta}^13*C~'(\u2030) range'), 
       fill = "Stream system")
  # annotate(geom = "text", x = 0.5, y = c(10, 10*0.9),
  #          hjust = 0, vjust = 1,  label = model_label, parse = TRUE, size = 3)
...
# PLot predictions
model_label <- c("s(PC1, 1.00)", "'Deviance expl.' == '24.9%'")
p.NR.c <- new_data %>% 
  ggplot(aes(x=PC1,y=fit)) + 
  geom_ribbon(aes(ymin=lower,ymax=upper, x = PC1), alpha=.5, fill="grey")+
  geom_line() +
  ...
  # scale_y_continuous(expand=c(0,0), limits=c(0,10), breaks = seq(0,10,2)) +
  labs(title = "", x = "Long. gradient (PC1)", 
       y = expression('Consumer '~{delta}^15*N~'(\u2030) range'), 
       fill = "Stream system") +
  annotate(geom = "text", x = 0.5, y = c(12, 12*0.95),
           hjust = 0, vjust = 1,  label = model_label, parse = TRUE, size = 3)
...
# Build panels
panel <- cowplot::plot_grid(
  p.kud.c + theme(legend.position=c(.75,.2)) + theme(plot.margin=unit(c(1,1,1,1),"mm")), 
  p.CR.c + theme(legend.position="none") + theme(plot.margin=unit(c(1,1,1,1),"mm")), 
  p.NR.c + theme(legend.position="none") + theme(plot.margin=unit(c(1,1,1,1),"mm")), 
  labels = c("A", "B", "C"),
  ncol = 3, nrow = 1, 
  align = "vh"
)
...
# Save it
path <- here::here("out", "r1_td_consumer_panel")
ggsave(glue::glue("{path}.pdf"), plot = last_plot(), 
...
panel <- cowplot::plot_grid(
  ...
  p.CR + theme(legend.position="none") + theme(plot.margin=unit(c(1,1,1,1),"mm")), 
  p.NR + theme(legend.position="none") + theme(plot.margin=unit(c(1,1,1,1),"mm")), 
  p.kud.c + theme(legend.position="none") + theme(plot.margin=unit(c(1,1,1,1),"mm")), 
  p.CR.c + theme(legend.position="none") + theme(plot.margin=unit(c(1,1,1,1),"mm")), 
  p.NR.c + theme(legend.position="none") + theme(plot.margin=unit(c(1,1,1,1),"mm")), 
...
# Save it
path <- here::here("out", "r1_td_panel_all")
ggsave(glue::glue("{path}.pdf"), plot = last_plot(), 
